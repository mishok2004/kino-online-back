...
export class RatingService {
    ...
    async averageRatingByMovie(modelId: Types.ObjectId | string) {
        const ratingsMovie: RatingModel[] = await this.RatingModel.aggregate().match({
            movieId: new Types.ObjectId(modelId)
        }).exec()

        return ratingsMovie.reduce((acc, el) => acc + el.value, 0) / ratingsMovie.length
    }
